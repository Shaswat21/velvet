...
const generateSVGString = (
  ...
  const renderObject = (obj: CanvasObject): string => {
    ...
    if (obj.type === "rect") {
      ...
      content = `
        <foreignObject width="${o.width}" height="${o.height}" style="overflow: visible;">
...
export default function DesignBoard({
  ...
  const board = useDesignBoard(paper, orientation, initialObjects, initialBgColor);
  ...
  const handleDownload = async (options: ExportOptions) => {
    ...
    if (format === "velvet") {
      const data = { objects: board.objects, paper, orientation, bgColor: board.bgColor };
    ...
    try {
      ...
      if (format === "svg") {
        ...
        const gifObjects = board.objects.filter((obj): obj is ImageObject => 
            obj.type === "image" && 
            ((obj as ImageObject).src.toLowerCase().includes("data:image/gif") || 
        ...
        const processedGifs = await Promise.all(
          gifObjects.map(async (obj) => {
            ...
            return {
              ...obj,
              frames,
              totalDuration: frames ? frames.reduce((acc, f) => acc + f.delay, 0) : 0,
            };
          })
        );
        const validAnimatedGifs = processedGifs.filter((g): g is ProcessedGif => g.frames !== null && g.frames.length > 0);
        ...
        const gifConfig = {
            ...
            filter: (node: HTMLElement) => {
                if (node.tagName === "IMG") {
                    ...
                    const isAnimated = validAnimatedGifs.some((g) => src.includes(g.src) || g.src.includes(src));
        ...
        if (ctx) {
            ...
            while (currentTime < DURATION_MS) {
                ...
                validAnimatedGifs.forEach((gifObj) => {
                    ...
                    ctx.drawImage(currentFrame, -(gifObj.width / 2) * scale, -(gifObj.height / 2) * scale, gifObj.width * scale, gifObj.height * scale);
                    ctx.restore();
                });
        ...
        let dataUrl = "";
        if (format === "jpg") {
          dataUrl = await htmlToImage.toJpeg(element, { ...config, quality: compress ? 0.6 : 1.0 });
        } else {
  ...
  return (
    ...
    >
      <Header
        ...
        selectedId={board.selectedIds.length === 1 ? board.selectedIds[0] : null}
        handleGroup={board.handleGroup}
        handleUngroup={board.handleUngroup}
        selectedCount={board.selectedIds.length}
        isGroupSelected={board.selectedIds.length === 1 && board.objects.find((o) => o.id === board.selectedIds[0])?.type === "group"}
      ...
      <CanvasArea
        ...
        addSelectedId={(id) => board.setSelectedIds((prev) => prev.includes(id) ? prev : [...prev, id])}
